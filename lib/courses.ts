const STORAGE_KEY = "net3almo-courses"
...
// Function to get courses from localStorage or use defaults
function getCoursesFromStorage(): Course[] {
  ...
  try {
    ...
    // If no courses in storage, initialize with defaults
...
// Export a function to get the latest courses
...
export function addCourse(course: Course) {
  const currentCourses = getCoursesFromStorage()
...
// Add function to get a course by ID
export function getCourseById(id: string): Course | undefined {
  const currentCourses = getCoursesFromStorage()
  return currentCourses.find((course) => course.id === id)
}

// Export courses for backward compatibility
export const courses = getCoursesFromStorage()
